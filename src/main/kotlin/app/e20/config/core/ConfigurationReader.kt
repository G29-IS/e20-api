            ...
            when (type) {
                ...
                Level::class -> {
                    try {
                        ...
                        throw IllegalArgumentException("Tried to read a value of type 'Level' for key '$key' but casting failed, value: $value")
                    }
                }
                else -> throw UnsupportedOperationException("Configuration reader required to read a value of type $type for key '$key' but no casting is implemented for that type")
            }
